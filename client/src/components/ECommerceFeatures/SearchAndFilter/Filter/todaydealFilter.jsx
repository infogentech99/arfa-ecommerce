...
// Component to display TodayDeal filtered products
const TodayDealFilter = () => {
  ...
  useEffect(() => {
    ...
    // Fetching the list of TodayDeal from the backend API
    axios
      .get("https://arfa-ecommerce.onrender.com/api/todaydeals")
      .then((res) => {
  ...
  const handleRemoveProduct = async (id) => {
    try {
      await axios.delete(
        `https://arfa-ecommerce.onrender.com/api/todaydeals/${id}`
      );
  ...
  return (
    <>
      <Header />
      <main className="flex">
        <FilterTitles todaydeal={"font-bold"} />
        ...
        <section className="flex-grow basis-4/5 p-2 sm:px-5 sm:pb-5">
          ...
          <div className="rounded-lg bg-white flex justify-around flex-wrap gap-1 sm:gap-5 my-1 sm:my-5">
            {products.map((item) => (
              ...
              >
                <img
                  className="h-28 sm:h-72 rounded-md cursor-pointer"
                  src={item.image}
                  alt={item.subtitle}
...
export default TodayDealFilter;
